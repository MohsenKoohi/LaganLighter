...
### Compiling and Executing Code
 - Set the path to `gcc` compiler folder in Line 1 of the `Makefile`, if you do not have it in `PATH`.
 - Please modify Line 9, if the required libraries (`libnuma` and `libpapi`) are not in the library path,`LD_LIBRARY_PATH`.
